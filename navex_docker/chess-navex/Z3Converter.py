...
class Z3Utils:
    def __init__(self):
        self.functionsWithNoModel = []

    def manageGenericNode(self, node, decl_vars, attackType):
        if node["formula"]["type"] == "AST_CALL":
            return self.manageFunctionCall(node, decl_vars, attackType)
        elif node["formula"]["type"] == 'AST_CONST':
            return self.manageGenericConstant(node, decl_vars)
        elif node["formula"]["type"] == 'AST_BINARY_OP':
            return self.manageBinaryOperation(node, decl_vars)
        elif node["formula"]["type"] == 'AST_UNARY_OP':
            return self.manageUnaryOperation(node, decl_vars)
        elif node["formula"]["type"] == 'AST_ASSIGN':
            return self.manageAssignmentOperation(node, decl_vars)
        elif node["formula"]["type"] == 'AST_CONTAINS':
            return self.manageContainmentOperation(node, decl_vars)


    def manageBinaryOperation(self, node, decl_vars):
        op1 = self.manageGenericOperandDecl(node["formula"]["right"][0], node["types"]["right"][0], decl_vars)
        op2 = self.manageGenericOperandDecl(node["formula"]["right"][1], node["types"]["right"][1], decl_vars)
        res = self.manageGenericOperandDecl(node["formula"]["left"], node["types"]["left"], decl_vars, True)
    ...
    def manageUnaryOperation(self, node, decl_vars):
        op = self.manageGenericOperandDecl(node["formula"]["right"], node["types"]["right"], decl_vars)
        res = self.manageGenericOperandDecl(node["formula"]["left"], node["types"]["left"], decl_vars, True)
    ...
    def manageAssignmentOperation(self, node, decl_vars):
        op = self.manageGenericOperandDecl(node["formula"]["right"], node["types"]["right"], decl_vars)
        res = self.manageGenericOperandDecl(node["formula"]["left"], node["types"]["left"], decl_vars, True)
    ...
    def manageContainmentOperation(self, node, decl_vars):
        op = self.manageGenericOperandDecl(node["formula"]["right"], node["types"]["right"], decl_vars)
        res = self.manageGenericOperandDecl(node["formula"]["left"], node["types"]["left"], decl_vars, True)
    ...
    def manageFunctionCall(self, node, decl_vars, attackType):
        model = self.getFunctionModel(node, decl_vars, attackType)
        if model != "": #we have no model  for this function. Ask the user.
            self.functionsWithNoModel.append(node)
            return model
        else:
            #		raise Exception()
            res = self.manageGenericOperandDecl(node["formula"]["left"], node["types"]["left"], decl_vars, True)
            ...
            for arg in node["formula"]["right"]:
                op = self.manageGenericOperandDecl(arg, node["types"]["left"], decl_vars)
                assertion = assertion + "(= " + res + " " + op + ")\n"
            assertion = assertion + "))"
            return assertion

    def manageGenericConstant(self, node, decl_vars):
        op = self.manageGenericOperandDecl(node["formula"]["right"], node["types"]["right"], decl_vars)
        res = self.manageGenericOperandDecl(node["formula"]["left"], node["types"]["left"], decl_vars, True)
        return "(assert (= " + res + " " + op + "))"

    def manageGenericOperandDecl(self, opName, opGuessedType, decl_vars, setAssigned=False):
        if opGuessedType == "":
            ...
            if opName not in decl_vars:
                ...
                if setAssigned:
                    if decl_vars[opName].assigned:
                        return self.getOperandWithConversion(decl_vars[opName].type, opGuessedType,
                                                        decl_vars[opName].getCompleteName(True))
                    else:
                        decl_vars[opName].setAssignment(True)
                        return self.getOperandWithConversion(decl_vars[opName].type, opGuessedType,
                                                        decl_vars[opName].getCompleteName())
        ...
        if isinstance(opName, str):
            ...
            return self.getOperandWithConversion("string", opGuessedType, opName)

        if isinstance(opName, bool):
            if opName:
                return self.getOperandWithConversion("boolean", opGuessedType, "true")
            else:
                return self.getOperandWithConversion("boolean", opGuessedType, "false")

        return self.getOperandWithConversion("number", opGuessedType, str(opName))


    def getFunctionModel(self, node, decl_vars, attackType):
        if node["formula"]["op"] == "mysql_query" and attackType == "sql":
            res = self.manageGenericOperandDecl(node["formula"]["left"], node["types"]["left"], decl_vars, True)
            query = self.manageGenericOperandDecl(node["formula"]["right"][-1], "string", decl_vars)
            ...
            res = self.manageGenericOperandDecl(node["formula"]["left"], "number", decl_vars, True)
            op = self.manageGenericOperandDecl(node["formula"]["right"][-1], "number", decl_vars)
            ...
            res = self.manageGenericOperandDecl(node["formula"]["left"], "boolean", decl_vars, True)
            op = self.manageGenericOperandDecl(node["formula"]["right"], "boolean", decl_vars)
            ...
            op = self.manageGenericOperandDecl(node["formula"]["right"], "string", decl_vars)
            ...
            res = self.manageGenericOperandDecl(node["formula"]["left"], "string", decl_vars, True)
            op = self.manageGenericOperandDecl(node["formula"]["right"][-1], "string", decl_vars)
            ...
            res = self.manageGenericOperandDecl(node["formula"]["left"], "string", decl_vars, True)
            search = self.manageGenericOperandDecl(node["formula"]["right"][-1], "string", decl_vars)
            subject = self.manageGenericOperandDecl(node["formula"]["right"][-3], "string", decl_vars)
    ...
    def getOperandWithConversion(self, fromType, toType, opName):
        if fromType == toType:
            return opName
    ...
    def generateFinalModel(self, decl_vars, assertions):
        res = ""
        ...
        for (v, o) in decl_vars.items():
            if (not o.assigned) and o.type == "string" and not self.containsSource(v):
