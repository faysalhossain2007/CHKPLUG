...
def main(argv):
   ...
   # vulVar = arvg[4].strip
    ...
    result, elapsed_time = sa.runTimedQuery(prepareQuery(attackType), True)
    ...
    for r in result:
        if r and isinstance(r, list):
            for l in r:
                printPath(l)
                vp = VulnerablePath(l)
                printVulnerablePathInTAC(vp)
                vp.solve(vp.pathInTAC)
...
def printVulnerablePathInTAC(vulnerablePath):
